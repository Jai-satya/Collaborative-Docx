  ...
  if (isLoading) {
    return (
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-32 rounded-xl bg-muted animate-pulse-subtle" />
        ))}
      </div>
    );
  }
  ...
  if (!documents?.length) {
    return (
      <div className="text-center py-20">
        ...
        <p className="font-body text-lg text-muted-foreground">No documents yet.</p>
        <p className="font-ui text-sm text-muted-foreground/60 mt-1">Create your first document to get started.</p>
      </div>
    );
  }
